...
class MyBarChart {
	...
	// Ho uaggiunto un parametro "title" modificando la classe
	setChartOptions(type, keysArray, valuesArray, colorsArray, title) {
		let maxValue = Math.max(valuesArray) + 1;
		let minValue = Math.min(valuesArray) + 1;
		...
		this.#chartOptions.options.scales.y.suggestedMax = maxValue
		this.#chartOptions.options.scales.y.suggestedMin = minValue
